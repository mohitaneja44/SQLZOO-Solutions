...
SELECT name from world where world.name like '%United%'; 

# 7
> Two ways to be big: A country is big if it has an area of more than 3 million sq km or it has a population of more than 250 million.
Show the countries that are big by area or big by population. Show name, population and area.

Select name, population, area from world where world.area > 3000000 or world.population > 250000000;
...
SELECT name
   FROM world
WHERE lower(name) NOT LIKE '% %' AND (name LIKE '%a%') AND (name LIKE '%e%') AND (name LIKE '%i%') AND (name LIKE '%o%') AND (name LIKE '%u%');
